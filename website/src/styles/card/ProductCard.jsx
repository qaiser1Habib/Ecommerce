...
import { addToWishListAsync } from "../../actions/wishlist";
...
const ProductCard = (props) => {
	...
	const handleCart = (product) => {
		if (items.findIndex((item) => item.product.id === product.id) < 0) {
			dispatch(addToCartAsync({ formData: { product: product.id, quantity: 1 }, notify }));
		} else {
			notify("error", "Product Already in Cart.");
		}
	};
	const handleWishList = (product) => {
		if (wishlist.findIndex((item) => item.product.id === product.id) < 0) {
			dispatch(addToWishListAsync({ formData: { product: product.id }, notify }));
		} else {
		...
		<>
			<div className="tp-product-item p-relative transition-3 mb-25">
				<div className="tp-product-thumb p-relative fix m-img">
					<Link to={`/product-detail`} state={{ product: product.id }}>
					...
					<div className="tp-product-action">
						<div className="tp-product-action-item d-flex flex-column">
							<button onClick={() => handleCart(product)} className="tp-product-action-btn tp-product-add-cart-btn">
